...
export default function Overview() {
  const { tasks, stats, loading, error, deleteTask } = useTasks()
  const [category, setCategory] = useState('All Categories')
  const [taskToDelete, setTaskToDelete] = useState(null)
  const navigate = useNavigate()

  const filtered =
    category === 'All Categories' ? tasks : tasks.filter((t) => t.category === category)

  if (loading) {
    return (
      <div className="flex-1 px-10 py-8">
        <p className="text-ledger-muted">Loading tasks...</p>
      </div>
    )
  }
                  ...
                  <td className="py-4">
                    <div className="flex items-center justify-end gap-3">
                      ...
                      <button
                        ...
                        className="text-ledger-ink/60 hover:text-red-500"
                      >
                        🗑
                      </button>
                    </div>
